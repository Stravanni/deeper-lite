...
def verify_split(df,
    ...
    for index, partition_df in enumerate([train_df, validation_df, test_df]):
        ...
        expected_duplicates, expected_non_duplicates = int(num_duplicates_df * split_ratio[index]), int(
...
def dataset_to_matrix(params,
    ...
    for row_index, row in candset_with_ids_df.iterrows():
        ...
        for col_index, attribute in enumerate(common_attributes):
            cosine_dist, normed_abs_dist = compute_distance(fasttext_model, ltable_row[attribute],
                                                            rtable_row[attribute])
